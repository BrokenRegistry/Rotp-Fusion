...
public interface Treasurer extends Base {
    default void allocateReserve(Empire empire) {
        ...
        if (empire.totalReserve() > 0) {
            List<StarSystem> remainingSystems = new ArrayList<>(systems);
			for (StarSystem sys : remainingSystems)
				sys.sortingValueFloat = sys.colony().production(); // to compute this only once per system
			Collections.sort(remainingSystems,StarSystem.INCREASING_FLOAT);
            for (StarSystem sys : remainingSystems) {
                if (empire.totalReserve() <= 0)
                    break;
                Colony col = sys.colony();
