...
public final class ColonyIndustry extends ColonySpendingCategory {
	...
	public float factoryToMaxRatio()	{ return factories / maxFactories(); }
    ...
    private float smoothSpendingNeeded() {
        float builtFactories = factories;
        int colonyControls = robotControls;
        float expectedMissingPopulation = max(0, planet().currentSize() - expectedPopulation());
...
//       	float upcomingPopGrowth  = colony().ecology().upcomingPopGrowth(); // Next Turn
	...
	public float[] factoryBalance()	{
		...
		float alienFactories	= convertableAlienFactories();
		float refitFlag = 0f;
