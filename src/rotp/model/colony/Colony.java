...
public final class Colony implements Base, IMappedObject, Serializable {
	...
	public void govFundColony(boolean reserve)	{ govAutoFundTag().toggle(reserve); }

    private boolean governorGotPlayerRequest()	{ return govOptions().isManageableGovernor(); }
    public boolean isObedientGovernor()			{ return govOptions().isManageableGovernor(); }
	...
	public boolean underSiege()					{ return underSiege; }		// Enemy fleet in orbit
	public float rawReserveIncome()				{ return reserveIncomeBC; }
    ...
    @Override public String toString() { return "Colony: " + printString();  }
	...
	public float starGateTimeToComplete()	 { return shipyard().starGateTimeToComplete(); }
	...
	public float totalPlanetaryExcessSpending(int id)	{	// To estimate tax revenue; Player only
		float totalBC = 0; 
		totalBC += shipyard().excessSpending()[id];
		totalBC += defense().excessSpending()[id];
		totalBC += industry().excessSpending()[id];
...
//        }
	...
	public static BufferedImage getBaseCoinImage(int width) {
		int w = width + width;
		...
		int margin = ScaledInteger.s6;
		int coinTL = margin;
		int coinW = w - margin - margin;
		int coinBR = coinTL + coinW;

		Image img = GameSession.instance().session().galaxy().player().flagNorm();
		...
		margin = ScaledInteger.s4;
		coinTL = margin;
		coinW = w - margin - margin;
		coinBR = coinTL + coinW;
		g.setStroke(ScaledInteger.stroke3);
	...
	public boolean isGrowing()	{ // Player Only
		if (planet.isResourceNormal()) {
			if (planet.isAntaran())
				return currentProductionCapacity() <= govOptions().autospendMaxIndRatioAntaran();
			if (planet.isOrionArtifact())
				return currentProductionCapacity() <= govOptions().autospendMaxIndRatioOrion();
			return currentProductionCapacity() <= govOptions().autospendMaxIndRatioNormal();
		}

		if (planet.isResourceRich())
			return currentProductionCapacity() <= govOptions().autospendMaxIndRatioRich();
		if (planet.isResourcePoor())
			return currentProductionCapacity() <= govOptions().autospendMaxIndRatioPoor();
		if (planet.isResourceUltraRich())
			return currentProductionCapacity() <= govOptions().autospendMaxIndRatioURich();
		if (planet.isResourceUltraPoor())
			return currentProductionCapacity() <= govOptions().autospendMaxIndRatioUPoor();
...
//			str += System.lineSeparator();
		...
		public boolean isNewColony()			{ return isGrowing(); }
...
//		public int reserveNeededBC()			{ return max (0, ceil(nextProduction - rawReserveIncome())); }
	...
	private final class GovAutoFundTag implements ScaledInteger, Serializable	{
		...
		private static final int CROSSED	= -10;
		...
		private int toggleMandate()	{
			...
			switch (fundingMandate) {
				...
				case GROWING:	if (hasOrders())	return ORDERS;
				case ORDERS:	if (hasUrgency())	return URGENCY;
				case URGENCY:	if (hasShipWork())	return SHIP_WORK;
				case SHIP_WORK:	return UNLIMITED;
				case UNLIMITED:	return CROSSED;
				case CROSSED:	return UNFUNDED;
			}
			return mandate;
		}
			...
			if (fundingMandate != UNFUNDED) {
				...
				switch (fundingMandate) {
					...
					case CROSSED:
						...
						int tl = s3;
						int w1 = w - tl - tl;
						g.drawOval(tl, tl, w1, w1);
						tl = w/4;
						w1 = w - tl;
						g.drawLine(tl, tl, w1, w1);
						g.dispose();
						return buffer;
				}
				if (useReserve) {
	...
	public static final class CoinAdvisor implements IAdvisor {
		public static  BufferedImage getSubsidiesInstruction ()	{
			...
			BufferedImage[] coins = {
					...
					GovAutoFundTag.getCoinImage(coinSide, species, GovAutoFundTag.CROSSED,	 false, false)
					};
