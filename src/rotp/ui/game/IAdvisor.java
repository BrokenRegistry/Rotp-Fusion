...
public interface IAdvisor extends ScaledInteger {
	...
	default void setBoxLocation(AdviceBox targetBox)	{
		...
		// relative position
		// find X location
		if (2*x + targetBox.width > iW) { // put box to the left textBox.x
			TEXT_BOX.x = Math.min(x - TEXT_BOX.width - xBoxShift, iW - TEXT_BOX.width);
			...
			TEXT_BOX.x = x + targetBox.width + xBoxShift;
		...
		// find Y location
		if (2*y + targetBox.height > iH) { // put box to the top textBox.y
			TEXT_BOX.y = y - TEXT_BOX.height - yBoxShift;
			...
			TEXT_BOX.y = y + targetBox.height + yBoxShift;
