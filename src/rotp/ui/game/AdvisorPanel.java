...
public final class AdvisorPanel extends BasePanel implements IAdvisor, IMapOverlay {
	...
	public void onHold()		{ releaseObjects();	}	// leave a panel
	...
	@Override public void paintOverMap(MainUI mainUI, GalaxyMapPanel mapPanel, Graphics2D g)	{
		A.mapHandler = mainUI;
		paintOverMap(g); }
	@Override public void paintComponent(Graphics g)	{ paintOverMap((Graphics2D) g); }
	...
	public void paintOverMap(Graphics2D g)	{
		...
		if (drawnTarget instanceof TechStatusSprite)
			((TechStatusSprite)drawnTarget).draw(map(), g);
		else if (drawnTarget instanceof TreasurySprite)
			((TreasurySprite)drawnTarget).draw(map(), g);
	}
