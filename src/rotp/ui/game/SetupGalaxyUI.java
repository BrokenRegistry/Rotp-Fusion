...
public final class SetupGalaxyUI  extends BaseModPanel implements MouseWheelListener {
	...
	private void initAIandAbilitiesList() {
		...
		specificAbilitiesList.addAll(SpeciesManager.current().getAllowedAlienCustomSpecies());
		...
		globalAbilitiesList.addAll(SpeciesManager.current().getAllowedAlienCustomSpecies());
	...
	private void updateRivalMugs(int i)	{
		...
		if (selOpp == null)
			...
			BufferedImage diplo = SpeciesManager.current().keyed(selOpp).diploMugshotQuiet();
...
//		txt  = dest.getGuide();
	...
	private BufferedImage playerRaceImg()	{
		if (playerMug == null) {
			...
			playerMug = newBufferedImage(SpeciesManager.current().keyed(selRace).diploMug());
		}
		return playerMug;
	}
	...
	@Override protected void initBackImg() {
		...
		if (opts.selectedPlayerIsCustom()) {
			race = SpeciesManager.current().keyed(CustomRaceDefinitions.CUSTOM_RACE_KEY, null);
			setupName = race.setupName;
		}
		else {
			race = SpeciesManager.current().keyed(opts.selectedPlayerRace());
			setupName = race.setupName();
		}
